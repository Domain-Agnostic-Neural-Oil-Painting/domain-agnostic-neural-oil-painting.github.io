...
from models.optims import LinearWarmupCosineLRScheduler


def get_scheduler(optimizer, opt):
    if opt.lr_policy == 'linear_warmup_cosine_lr':
        # def lambda_rule(epoch):
        #     # # lr_l = 1.0 - max(0, epoch + opt.epoch_count - opt.n_epochs) / float(opt.n_epochs_decay + 1)
        #     lr_l = 0.3 ** max(0, (epoch + opt.epoch_count - opt.n_epochs) // 5)
        #     # lr_l = 0.3 ** (epoch // 2)  # 每2个epoch更新一次
        #     # print(f"Epoch: {epoch}, Lambda: {lr_l}")  # 打印每个epoch的lambda值
        #     return lr_l
        ...
        warmup_start_lr = opt.warmup_start_lr
        # 创建调度器实例
