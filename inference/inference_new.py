...
# 定义TTA的损失函数
def tta_loss(output, target, layer_size):
    loss_pixel = torch.tensor(0., device=target.device)
    criterion_pixel = torch.nn.L1Loss().to(target.device)
    target_shape = (1, 3, layer_size, layer_size)
    target_new = target.view(*target_shape)
    target_resized = F.interpolate(target_new, size=(output.size()[2], output.size()[3]), mode='bilinear', align_corners=False)
    # print("target_resized:", target_resized.shape)
    # print("output:", output.shape)
    loss_pixel = criterion_pixel(output, target_resized) * 10
    return loss_pixel
    
# test_time_adapt 函数
def test_time_adapt(model, img_patch, result_patch, final_result, meta_brushes, layer_size, patch_num, optimizer, original_h, original_w, frame_dir,
                    serial=True, tta_steps=1, stroke_num = 8, patch_size = 32):
    prev_loss = float('inf')
    model.zero_grad()
    for step in range(tta_steps):
        
        # print(f"Before model: img_patch.requires_grad={img_patch.requires_grad}, result_patch.requires_grad={result_patch.requires_grad}")
        shape_param, stroke_decision = model(img_patch, result_patch)
        # print(f"After model: shape_param.requires_grad={shape_param.requires_grad}, stroke_decision.requires_grad={stroke_decision.requires_grad}")
        stroke_decision = network_origin.SignWithSigmoidGrad.apply(stroke_decision)
        ...
        param_clone = param.clone().detach()
        param_clone.requires_grad = True
        param_modified = param_clone.clone()
        param_modified[..., :2] = param_modified[..., :2] / 2 + 0.25
        param_modified[..., 2:4] = param_modified[..., 2:4] / 2

        # 渲染图像
        if serial:
            final_result = param2img_serial(param_modified, decision, meta_brushes, final_result, frame_dir, False,
                                            original_h, original_w)
        else:
            final_result = param2img_parallel(param, decision, meta_brushes, final_result)

        # 计算损失
        loss = tta_loss(final_result, img_patch, layer_size)
        loss.backward(retain_graph=True)
        torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0)
        optimizer.step()
        optimizer.zero_grad()
        if abs(prev_loss - loss.item()) < 1e-4:  # 你可以调整这个阈值
            break
        prev_loss = loss.item()
        if step % 2 == 0:  # 每两步打印一次
            print(f"TTA Step {step}, Loss: {loss.item()}")

    return model, final_result, loss.item()

def main(input_path, model_path, output_dir, need_animation=True, resize_h=None, resize_w=None, serial=True):
    ...
    net_g = network_origin.Painter(5, stroke_num, 256, 8, 3, 3).to(device)
    net_g.load_state_dict(torch.load(model_path))
    
    for name, param in net_g.named_parameters():
        if 'norm' not in name.lower() and 'bn' not in name.lower():
            param.requires_grad = False
    for name, param in net_g.named_parameters():
        if not param.requires_grad:
            param.grad = None
    optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, net_g.parameters()), lr=1e-4, betas=(0.9, 0.999))
    # optimizer = torch.optim.Adam(net_g.parameters(), lr=1e-4, betas=(0.9, 0.999))
    net_g.train()
    ...
    # print(original_h, original_w)  # 打印 original_h 和 original_w 的值
    ...
    for layer in range(0, K + 1):
        ...
        # 在这里应用 TTA
        net_g, final_result, tta_loss = test_time_adapt(net_g, img_patch, result_patch, final_result, meta_brushes, layer_size, patch_num, optimizer, original_h, original_w, frame_dir)  # 使用 img_patch 作为目标
        # # 保存每层的中间结果
        # save_img(final_result.squeeze(), f"{frame_dir}/layer_{layer}_result.png")
