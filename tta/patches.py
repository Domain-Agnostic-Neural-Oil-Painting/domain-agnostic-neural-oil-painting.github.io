...
from models import network_origin
...
class PatchDataset(Dataset):
    ...
    def __getitem__(self, idx):
        img_patch, result_patch = self.patches[idx]
        return img_patch, result_patch
    

    
def create_patches(input_path, model_path = 'model.pth', num_augmentations=4):
    resize_h=None
    resize_w=None
    patch_size = 32
    patches = []
    ...
    with torch.no_grad():
        ...
        for layer in range(0, K + 1):
            ...
            for i in range(img_patch.shape[0]):
                patches.append((img_patch[i], result_patch[i]))  # Ensure all patches are added as pairs    
            ...
            if serial:
                final_result = param2img_serial(param, decision, meta_brushes, final_result,
                                                None, False, original_h, original_w)
            else:
        ...
        # Add the final img_patch and result_patch to patches
        for i in range(img_patch.shape[0]):
            patches.append((img_patch[i], result_patch[i]))
        
    random.shuffle(patches)
        
    return patches

# input_path = '../inference/input/chicago.jpg'
# patches = create_patches(input_path)
